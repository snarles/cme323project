...
In practice since $\epsilon(M - 1)$ is too conversative, we manually choose $\epsilon$ and add a truncation step as a safeguard.

```{r}
fw_greedy <- function(A, M, epsilon = epsilon_M[M - 1]) {
  n <- dim(A)[1]
  S <- A
  P <- cell(n)
  rownames(P) <- rownames(A); colnames(P) <- colnames(A)
    ...
    for (i in 1:n) {
      for (j in 1:n) {
        if (S[i, j] > S[i, k] + S[k, j]) {
          ...
          v <- c(v1, v2[-1])
          # greedy step
          if ((length(v) > M + 2) && (epsilon_check(v, epsilon))) {
            v <- epsilon_compress(v, epsilon)
          }
          # in case greedy failed due to epsilon < epsilon(M - 1)
          if (length(v2) > M + 2) {
            # truncate
            v <- c(v[1], v[2:(M+1)], v[length(v)])
          }
          P[[i, j]] <- v
        }
      }
...
Greedy with $M = 3$ and $\epsilon = 0.4$
```{r}
res_g <- fw_greedy(A, 3, 0.4)
# the full path
P[[i, j]]
# the stored path
res_g$P[[i, j]]
